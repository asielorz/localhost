...
async fn process_request(req : Request<Body>) -> Result<Response<Body>, hyper::Error> 
{
    ...
    match (req.method(), req.uri().path()) {
        ...
        (&Method::PUT, path) if paths::is_single_entry_path(path) => requests::put_single_text(req).await,
        (&Method::DELETE, path) if paths::is_single_entry_path(path) => requests::delete_single_text(req),
        (&Method::GET, path) if paths::is_entry_image_path(path) => requests::get_entry_image(req),
        (&Method::PUT, path) if paths::is_entry_image_path(path) => requests::put_entry_image(req).await,
        (&Method::DELETE, path) if paths::is_entry_image_path(path) => requests::delete_entry_image(req),
        (&Method::GET, path) if paths::is_entry_backup_path(path) => requests::get_entry_backup(req),
        (&Method::PUT, path) if paths::is_entry_backup_path(path) => requests::put_entry_backup(req).await,
        (&Method::DELETE, path) if paths::is_entry_backup_path(path) => requests::delete_entry_backup(req),
